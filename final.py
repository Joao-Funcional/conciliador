...
from typing import List, Literal, Tuple, Optional, Dict, Callable
...
# Cada item: (nome_regra, função_condicao -> pl.Expr, deslocamento_em_dias)
def _cond_category_id(cat: str):
    return lambda: pl.col("categoryid") == cat


def _cond_cat_op(cat: str, op_upper: str):
    return lambda: (pl.col("categoryid") == cat) & (pl.col("api_optype_upper") == op_upper)


def _cond_category_lower(cat_lower: str):
    return lambda: pl.col("api_category_lower") == cat_lower


DATE_SHIFT_RULES: list[tuple[str, Callable[[], pl.Expr], int]] = [
    ("cat_05070000_tarifa_avulsos", _cond_cat_op("05070000", "TARIFA_SERVICOS_AVULSOS"), -2),
    ("op_rendimento_aplic_financeira", lambda: pl.col("api_optype_upper") == "RENDIMENTO_APLIC_FINANCEIRA", -1),
    ("cat_03060000", _cond_category_id("03060000"), -2),
    ("cat_05050000", _cond_category_id("05050000"), -1),
    ("cat_15030000", _cond_category_id("15030000"), -1),
    ("cat_16000000", _cond_category_id("16000000"), -1),
    ("api_tax", lambda: pl.col("api_is_tax"), -1),
    ("api_bankfees", lambda: pl.col("api_is_bankfees"), -1),
    ("api_pix_tariff", lambda: pl.col("api_is_pix_tariff"), -2),
    ("api_rent", lambda: pl.col("api_is_rent"), -2),
    ("category_transfer_fx", _cond_category_lower("transfer - foreign exchange"), -1),
]
...
def load_api_from_pg() -> pl.DataFrame:
    ...
    df = (
        ...
        .with_columns([
            ...
            pl.col("date_br").dt.date().alias("api_date"),
    ...
    base_date_expr = pl.col("api_date")
    date_expr = None
    for _name, cond_fn, delta in DATE_SHIFT_RULES:
        cond = cond_fn()
        shifted = base_date_expr + pl.duration(days=delta)
        date_expr = (
            pl.when(cond).then(shifted)
            if date_expr is None
            else date_expr.when(cond).then(shifted)
        )

    api_date_expr = (date_expr.otherwise(base_date_expr) if date_expr is not None else base_date_expr)
